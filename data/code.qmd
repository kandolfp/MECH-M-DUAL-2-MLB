...
This means we can filter between actual changes to the structure and source and a commit for a _simple experiment_.
...
One possible interpretation of the config (among many others is) can be seen in the next code block.
...
Like always, the _best_ platform depends on the project and the available infrastructure.
:::
...
To show some of the `dvclive` features we reworked to code, see commit [7f407b](https://github.com/kandolfp/MECH-M-DUAL-2-MLB-DATA/tree/7f407bab7bfcc191014eeb3b939bb2fe0456fb58) for the changes (including `pdm add dvclive`).
...
If a storage that can be accessed via WebDAV[^webdav] is available is it or use _local storage_ as an alternative, see [File systems (local remotes)](https://dvc.org/doc/user-guide/data-management/remote-storage#file-systems-local-remotes). 
:::
...
1. On the course _sakai cloud_ your have write rights in the subfolder `students_remote`, create a folder with your name and use it as remote.
  - Synchronize the the performed experiments with the remote (`dvc` and `git`).
