...
The pipeline features we are after are part of `dvc stage` command and we add our training

```bash
MLB-DATA$ pdm run dvc stage add --name train \
          --deps data --deps src/MECH-M-DUAL-2-MLB-DATA/ \
          --deps params.yaml --outs dvclive \
...
Before we follow the instructions to we need to run 

```bash
MLB-DATA$ pdm run dvc commit
```
to create the `dvc.lock` file associated with the stage.
It keeps track of the dependencies:
```yaml
schema: '2.0'
stages:
  train:
...
See [1282a7e9](https://github.com/kandolfp/MECH-M-DUAL-2-MLB-DATA/tree/1282a7e926e17d4a5ac2f02a30764bfd1026872f) and how this is reflected in our reference repository.
...
As we did no change anything in our configuration (see pependencies) `dvc` is smart enough to basically just copy the experiment.
But we can also change the parameters interactively, e.g.

```bash
MLB-DATA$ pdm run dvc exp run --set-param 'PCA.init_args.n_components=5'
Reproducing experiment 'sappy-corm'
Building workspace index                             |4.00 [00:00,  378entry/s]
...
This is reflected in the experiment list 
...
If we want to look at a specific experiment we can use `dvc exp apply` to restore it.

In order to share experiments we need to push them to the (git) remote.
This is done via
...
The biggest thing missing for our configuration with `dvc` is a remote to store our data. 

::: {.callout-important}
For this lecture we use the storage on [Sakai](https://sakai.mci4me.at/), this also means that this part is tricky to follow if you do not have something like this.

In such a case use local storage as an alternative: [File systems (local remotes)](https://dvc.org/doc/user-guide/data-management/remote-storage#file-systems-local-remotes). 
:::

We can add the remote as
```bash
...
```
this create the file `.dvc/config` with the following content:
...
To nicely separate the sensitive information there also exist a `.dvc/config.local` file that is in the `.gitignore` and will not be committed. 
We add our user information as

```bash
pdm run dvc remote modify --local myremote user ***
pdm run dvc remote modify --local myremote password ***
```
(the user is without the `@mci4me.at`)

To handle webdav `dvc` requires the package `dvc-webdav`, install it via `pdm`.
See [afe0848](https://github.com/kandolfp/MECH-M-DUAL-2-MLB-DATA/tree/afe08488167a3a68788f67b64cbd76e7261deee6) and how this is reflected in our reference repository.

Now we can run `dvc push` and our data is stored remote
```bash
