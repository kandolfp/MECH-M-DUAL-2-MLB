...
Now that we know how to persist our models, or hope to do so, we need to talk about how we keep track of what exactly we stored.

In the previous exercises we created multiple versions of our model and stored them to.
If we look at the different files, are we sure we know which version corresponds to which code block?
If we can, good job but what about in two weeks?

Now if we try with various parameters for our composite method to achieve better results we might end up with even more variants. 
To make sure we have reproducible models we need to track the model together with the code.
This is what we are going go look at in the next section. 