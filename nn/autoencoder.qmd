...
Autoencoders can be understood to generalize the linear subspace embedding of _singular value decomposition_/_principal component analysis_[^1] to a nonlinear manifold embedding.
...
Crucially, $z$ is usually a low dimensional space, represents the output of the encoding and the input of the decoding part of the NN.
This will force the latent space to reveal some useful properties that we can study.

::: {#fig-nn-autoenc-image}
![](../_assets/nn/autoencoder)

General structure of an _autoencoder_ with the _encoder_ network on the left and the _decoder_ network on the right (they don't need to be looking the same).
:::
...
::: {.callout-note}
We can use this to span the same space as PCA.
...
For nonlinear functions $f$ and $g$ a more general form than PCA is learned.
:::

To connect this concept back to what we discussed in the _Eigenfaces Example_[^eigenface], where we used truncated SVD to project into a low dimensional space and than back to reconstruct the image, we formulate it in a more mathematical fashion. 

We call the encode $y=f_\Theta(x)$ and the decoder $z=g_\Theta(y)$. 
...
This loss function is most of the time including some more general constraints to enforce something like sparsity, similar as LASSO or RIDGE, this is often referred to _regularized autoencoders_.

[^1]: see @Kandolf_GDM, Section 4 for SVD and 4.2 for PCA or follow the direct [link](https://kandolfp.github.io/MECH-M-DUAL-1-DBM/matrixdc/svd.html).
...
To illustrate this we construct an autoencoder for denoising the MNIST dataset we have seen before.

The first part is the data preparation.
```{python}
...
def transform(X, noise=0.4):
    X_org = torch.tensor(X / 255.0).to(torch.float)
    X_noise = torch.tensor(np.random.normal(0.5, noise, X_org.shape)).to(torch.float) + X_org
...
for j in range(length):
    im[0:28, 28*j:28*(j+1)] = y[j].reshape(28, 28)
    im[28:28*2, 28*j:28*(j+1)] = x[j].reshape(28, 28)
    im[28*2:28*3, 28*j:28*(j+1)] = z[j].reshape(28, 28)
...
Rework the `transform` function to make it more complicated.
In addition to the noise add:
...
Retrain the autoencoder with these training sets and see how it performs.
::::
:::