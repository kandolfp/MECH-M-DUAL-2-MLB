...
If we extend again to a multilayer NN with multiple neutrons in each layer we can simplify this a bit by limiting us for a moment to a linear NN.
In this case the functions $f_k$ are all linear maps and the composition is just the matrix-matrix computation
$$
y = A_m A_{m-1} \cdots A_2 A_1 x.
$$
The matrices might be sparse or dense, depending on the connections they describe.
The resulting system is most likely under-determined and requires some constraints to select a _unique solution_.

We can investigate this further by considering a one layer NN for our cats and dogs example with the following structure.
...
And we can see that a single layer NN can be used to build a least-square fit.

Of course there are other possibilities to solve this system (as discussed in @Kandolf_GDM), e.g. the LASSO method or RIDGE.
Let us test this. 
...
lsq = linear_model.LinearRegression().fit(X_train, y_train)
