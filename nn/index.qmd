...
If we extend again to a multilayer NN with multiple neurons in each layer we can simplify this a bit by limiting us for a moment to a linear NN.
In this case the functions $f_k$ are all linear maps (we will see shortly it is in fact the identity) and the composition is just the matrix-matrix computation
$$
