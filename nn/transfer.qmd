...
As mentioned, if our training set is small is small, this is especially beneficial.

The main scenarios for transfer learning are (compare @):
...
We can now continue training with our data set. 
...
- if the new data set is small we should not not fine-tune but rather only train a classifier.

    - if the data set is very different for the original training set we should probably remove a couple of layers and not the final stage.

- if we have a large dataset we can risc fine-tuning of the network as overfitting is limited.
    
    - if the data set is very different from the original training set we can also train the entire model from scratch but we should still consider initializing with existing weights so we do not need to relearn everything (e.g. edges).

## Fine-tuning an ImageNet for cats and dogs

As example, we use our already well known dogs and cats example.
As mentioned before, the data set is small and we know that the ImageNet has the capabilities to classify for different species in dogs and cats.
...
As mentioned, we want a model trained on the [ImageNet](https://www.image-net.org/index.php) data set, we use EfficientNet (@efficientnet). We will see how to load the model a bit later but it is important to have a look at the [docs](https://pytorch.org/vision/stable/models/efficientnet.html) to get an idea how we need to prepare our images and how to replace the final hidden layer.
...
As the ImageNet data set uses _normal_ images we do not use our images in the wavelet basis but our original images.

```{python}
#| code-fold: true
#| code-summary: "Show the code for loading the data set"
import numpy as np
import scipy
import requests
import io
...
To do this we use some helper functions from the `torchvision` module.


```{python}
T = tv.transforms.Compose([                                         # <1>
    tv.transforms.ToPILImage(),                                     # <2>
    #tv.transforms.Pad((96, 96, 96, 96), fill=255),
    tv.transforms.Grayscale(num_output_channels=3),                 # <3>
    tv.transforms.Resize((256, 256)),                               # <4>
    tv.transforms.ToTensor(),
    tv.transforms.Normalize(mean=[0.485, 0.456, 0.406],             # <5>
...
The functions for preparing the training and validation data set, the training loop and plotting stay the same. 
...
Instead of bloating up the image via `Resize` use `Pad` to imped the image with a boundary to make sure to reach the original size constraints.
