...
One aspect if machine learning is binning data into meaningful and distinct categories that can be used for decision making.
This is often called clustering and classification. 
...
One way is to extract the dominate features in a data set are for example the singular value decomposition (SVD) or principal component analysis, see [@Kandolf_GDM].
The idea is to not work in the heigh dimensional measurement space, but instead in the low rank feature space.
This allows us to significantly reduce the cost by performing the our analysis in this low dimensional space. 
...
This is often used to discover previously unknown patterns in the (low-rank) subspace of the data and leads to _feature engineering_ or _feature extraction_. 
These features can than be used for building a model.

Supervised learning, on the other hand, uses labelled data sets.
The training data is labelled for _cross-validated_ by a teacher or expert.
...
There are several different form of supervised learning like _reinforcement learning_, _semi-supervised learning_, or _active learning_. 
...
### Fischer Iris data set {#sec-fischer-iris-data-set}
First we look into one of the standard data sets well known in the community, the so called Fischer Iris data set @Fischer.

::: {.callout-note}
Luckily for us there are several data packages available for general use.
One of them is the [Fisher Iris data set](https://en.wikipedia.org/wiki/Iris_flower_data_set).

:::: {#fig-medium-iris}
[![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*H2UmG5L1I5bzFCW006N5Ag.png)](https://eminebozkus.medium.com/exploring-the-iris-flower-dataset-4e000bcc266c)

Illustration of the recorded features and structure of the iris dataset from the [Medium article - Exploring the Iris flower dataset](https://eminebozkus.medium.com/exploring-the-iris-flower-dataset-4e000bcc266c).
::::
...
#| fig-cap: "Iris data set with 150 samples from three distinct species, _Setosa_, _Versicolor_, and _Virginica_, as a scatter 3D plot along the features sepal length, sepal width, and petal width, the forth feature petal width is not shown."
#| code-fold: true
...
The petal length seems to be the best feature for the classification and no highly sophisticated machine learning algorithms are required.
We can see the this even better if we use a so called Pair plot from the [`seaborn`](https://seaborn.pydata.org/) package as seen in @fig-clustering-iris2.
...
Therefore, we can see the combination of two and in the diagonal we see the univariate distribution of the values.
After a close inspection we can see that _petal width_ provides us with the best distinction feature and _sepal width_ with the worst.

Even though we already have enough to give a good classification for this dataset it is worth to investigate a bit further in order to illustrate how, for example the principal component analysis (PCA) [see @Kandolf_GDM, Section 4.2] [Link](https://kandolfp.github.io/MECH-M-DUAL-1-DBM/matrixdc/svd.html#sec-matrixdc-pca) can be applied here.
...
plt.show()
```
As can be seen in the @fig-clustering-iris-pca the first two components cover about `{python} float(np.round(q*100, 2))`% of the total variation in the samples and in accordance with this result a very good separation of the three species.
...
As we can see with the Iris data set we often have a problem of _dimensionality_ if we try to visualize something.
...
- Linear discriminant analysis (LDA) - projects data onto a hyperplane where the hyperplane is chosen that is _most discriminative_ for the observations.
:::
...
In this data set we explore how distinguish between dogs and cats.
The data set contains $80$ images of dogs and $80$ images of cats, each with $64 \times 64$ pixels and therefore a total of $4096$ feature measurements per image. 

::: {.callout-important}
The following data set and the basic structure of the code is from [@Brunton2022, Code 5.2 - 5.4].
...
#| fig-cap:
#|   - "The first 16 cats from the data set."
#|   - "The first 16 dogs from the data set."
#| layout-ncol: 2
#| code-fold: true
#| code-summary: "Show the code for the figure"
import numpy as np
...
While mode 2 (@fig-clustering-dvc-pca-2) especially features the pointy ears common in cats, mode 3 (@fig-clustering-dvc-pca-3) is more dog like.
Consequently, will the two different animals have either add or subtract this feature form the mean.
...
For this let us first see how easy it is to distinguish the two classes in a scatter plot for the first three modes. 
