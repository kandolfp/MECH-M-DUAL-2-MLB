...
One such way is to extract the dominate features in a data set is for example the singular value decomposition (SVD) or principal component analysis, see [@Kandolf_GDM].
The main idea is to not work in the heigh dimensional measurement space, but instead in the low rank feature space.
...
As we can see with the Fischer Iris data set, we often have a problem of _dimensionality_, if we try to visualize something.
...
In this data set we explore how to distinguish between images dogs and cats, more particular the head only.
...
As before, a general inspection of the data set is always a good idea.
