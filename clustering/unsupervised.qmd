...
The goal of unsupervised learning is to discover clusters in the data of observations that have no labels, i.e. we have nothing to look at as reference.
There are several algorithms to perform this task, the most prominent is the $k$-means clustering algorithm.

::: {.callout-tip}
We will use `scikit-learn` in this section heavily.
The [User Guide](https://scikit-learn.org/stable/user_guide.html) and [Examples](https://scikit-learn.org/stable/auto_examples/index.html) sections in the documentation are well structured and a good source for additional information. 

Furthermore, there are several Examples that compare different methods on different datasets and therefore give a nice overview on strengths and weaknesses.
Have look at them to gain further insights into the methods and their differences.
...
The $k$-means algorithm tries to partition a set of $m$ (vector-valued) _data observations_ into $k$ clusters.
...
1. Repeat, starting from step 2, until the cluster centers stay stable up to some tolerance.
...
def lloyd(data, centers, steps):
    ...
    for i in range(steps):
        for j in range(data.shape[0]):
            classes[j] = np.argmin(np.linalg.norm(centers_ - 
                                   np.tile(data[j, :],
                                       (centers.shape[0], 1)),
                                   axis=1))
...
::: {.callout-important}
The following dataset and the basic structure of the code is from [@Brunton2022, Code 5.5 - 5.6].
...
#|   - "Point association to the clusters in the eighth step."
#| code-fold: true
#| layout-ncol: 2
...
In @fig-clustering-unsupervised_lloyd-1 we see the two _distinct_ clusters and the initial guesses for the centers.
...
As can be seen in @fig-clustering-unsupervised_lloyd2-1 the algorithm comes up with (almost) the same split between the two sets as our crude implementation.
...
## Apply the $k$-means algorithm to other datasets

As an exercise, to get some practice for using $k$-means, apply the algorithm to some other datasets to see how it performs.

1. We already looked at the Fischer Iris dataset (see the introduction this part of the notes) and discussed some basic features. Try with only two dimensional data, as well as four dimensional data to find the three clusters.

1. For @fig-clustering-unsupervised_dbscan we will look at the moons dataset.
Try working out the clusters in this case.

1. Apply the algorithm to the cats and dogs (in wavelet basis, see the introduction this part of the notes) for various principal components and find the two clusters.
::::
:::
...
1. **Image segmentation**: To decompose an image into different regions we can use $k$-means. Applications range from robotics to surveillance, where one or more objects are separated from the rest of the image.
...
There exist more sophisticated methods in finance, but $k$-means is an easy to understand start.
...
1. **Quality control**: By grouping similar products we can detect quality issues and defects in production processes. If an issue is found the part can be checked and the process adjusted.

1. **Traffic Analysis**: In transport and logistics we can analyze traffic patterns and use the information for optimization and similar trips, routes, and vehicles.
...
In @fig-clustering-unsupervised-img-seg and the corresponding code we show how to perform colour segmentation with $k$-means.

::: {.callout-important}
The following dataset and the basic structure of the code is from @Geron2022-xh, see [GitHub](https://github.com/ageron/handson-ml3/blob/main/09_unsupervised_learning.ipynb), 
:::

```{python}
#| label: fig-clustering-unsupervised-img-seg
#| fig-cap: "Colour segmentation for the image of a ladybug."
#| fig-subcap: 
#|   - "Original image."
...
The resulting tree allows us to easily see if data is clustered without the need of labeling or supervision.

We follow the example and discussion given in [@Brunton2022, Section 5.4]. 

There are two main approaches in creating the desired hierarchy: bottom-up often called _agglomerative_,
top-down often called _divisive_.
...
To illustrate the agglomerative approach and the difference in norms we use four points and construct the hierarchy by combining the two closest points, see @fig-clustering-unsupervised-dendrogram-explained-1 and @fig-clustering-unsupervised-dendrogram-explained-2.

::: {#fig-clustering-unsupervised-dendrogram-explained}

![Use two norm or euclidean norm to compute the distance - $\| \cdot \|_2$](../_assets/clustering/dendrogram_euclidean){#fig-clustering-unsupervised-dendrogram-explained-1 width=4.58in}

![Use one or cityblock norm to compute the distance - $\| \cdot \|_1$](../_assets/clustering/dendrogram_city){#fig-clustering-unsupervised-dendrogram-explained-2 width=4.62in}
...
It can also include the distance.
:::
...
#| fig-cap: "Construction a agglomerative hierarchy for our dataset."
#| fig-subcap: 
#|   - "Dendrogram for euclidean norm."
#|   - "Histogram for euclidean norm."
#|   - "Dendrogram for cityblock norm."
#|   - "Histogram for cityblock norm."
#| code-fold: true
#| layout-ncol: 2
#| code-summary: "Show the code for the figure"
import numpy as np
...
The two dendrograms (@fig-clustering-unsupervised_dengrogram-1 and @fig-clustering-unsupervised_dengrogram-3) show the hierarchical structure derived from the dataset.
...
The two bar graphs on the right (@fig-clustering-unsupervised_dengrogram-2 and @fig-clustering-unsupervised_dengrogram-4) show how the observations is clustered in the dendrogram. 
...
If we recall, that the first 25 points are from set 1 and the next 25 from set 2 we can see that the euclidean distance and the _cityblock_ norm place one point in the wrong cluster.
...
::: {.callout-note}
In `scikit-learn` this is supported via the [`sklearn.cluster.AgglomerativeClustering`](https://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html#sklearn.cluster.AgglomerativeClustering) class.
:::

::: {.callout-caution appearance="simple" icon=false}
:::: {#exr-dendrogram}

## Apply the agglomerative clustering to the Fischer Iris dataset

1. Work out how to use the `scikit-learn` alternative.

1. We already looked at the Fischer Iris dataset (see the introduction this part of the notes) and discussed some basic features. Try with only two dimensional data, as well as four dimensional data, to find the three clusters.

1. For @fig-clustering-unsupervised_dbscan we will look at the moons dataset.
Try working out the clusters in this case, look at the different possibilities with the `linkage` option as `{"ward", "complete", "average", "single"}`.
...
Our observations (we will call them points here for consistency) form the basis and they can be in any space but as for all the algorithms presented here, there needs to exist some way to measure distance.
...
1. A point $q$ is called _directly reachable_ from a core point $p$ iff[^iff] $q$ is in the $\epsilon$-neighbourhood of $p$.
...
In contrast to $k$-means the algorithm we can find _non-linear_ (i.e. curved) clusters.
We illustrate this with the _moon dataset_. 
...
As all algorithms depending on distance measures, it suffers from the curse of dimensionality[^dim].
...
1. Apply the DBSCAN algorithm to our toy example (the two ellipsoids from @sec-clustering-usl-kmeans) used through this section, to recover the two clusters as good as possible.
Try different $\epsilon$ values as well as different norms (`metric` parameter).

1. Additionally, for a higher dimensional problem, using DBSCAN to split the Fischer Iris dataset (see the introduction this part of the notes) into its three clusters of flowers.
::::
:::
...
This is often quite a sensible choice, if nothing more is known about the observations, and a Gaussian distribution is therefore a save bet.
...
The parameters are then iteratively updated to find a (local) maximum.
There are some problems with this approach, e.g. if we set one of the processes to have zero variance and mean equal to a point.
...
Lets follow the derivative for the GMM method in [@Brunton2022, Section 5.5] to get a better idea what is happening. 
...
We transform it into an optimization problem by setting the derivative to zero
$$
...
Therefore, we start of by assuming an initial guess of the vector $\Theta$ and this leads to the _posterior probability of distribution $p$ of $x_j$
$$
...
Let us try it with the cats and dogs images in wavelet basis from the introduction by fitting to the second and forth principal value of the SVD.
Note, we directly load the images in wavelet basis and do not recompute them.

::: {.callout-important}
The following dataset and the basic structure of the code is from [@Brunton2022, Code 5.8].
Also see [GitHub](https://github.com/dynamicslab/databook_python).
:::
...
If the algorithm struggles to recover the clusters, we can help it by imposing the shape of the cluster via restricting the covariance matrix $\Sigma_p$.
...
1. Apply the GMM algorithm to our toy example (the two ellipsoids from @sec-clustering-usl-kmeans) used through this section to recover the two clusters as good as possible.
...
1. Additionally, for a higher dimensional problem, split the Fischer Iris dataset into its three clusters of flowers.

1. Try to use GMM to get clusters for the moon dataset used in @sec-clustering-ul-dbscan.
...
For this the threshold needs to be defined in accordance to our expected anomalies. 
...
We do this with the (in)famous MNIST dataset of hand written digits. 
Lets start by looking at the dataset and establishing some basic properties.
...
#| echo: false
import textwrap
for paragraph in mnist.DESCR.split('\n'):
    print(textwrap.fill(paragraph, width=70))
```
:::
::: {.content-visible unless-format="html"}
:::: {.callout-note}
We suppressed the output in this format of the notes.
Please use the above code to view it.
It is not a problem to continue reading without reading the description first.
::::
:::
...
Also the `2` digit comes in various forms, with loop (1st row, 6th column) and without (3rd row, 6th column).
Finally we all know that a `3` can hide in an `8`, same as a `7` and `1` can hide in a `9` and much more. 
...
In general, they are, as expected, smudged as they do not represent an actual image.
...
Obviously this is not a good way to proceed and to find our digits.
...
[^dim]: Problems occurring for higher dimensional data and distance measures, see [WikiPedia](https://en.wikipedia.org/wiki/Curse_of_dimensionality#Distance_functions)
