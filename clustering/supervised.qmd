...
If we recall @Fischer from the Iris dataset, we can also find one of the first supervised learning methods in this paper.
...
::: {.callout-important}
The following introduction and illustrational dataset, as well as the basic structure of the code is from [@Brunton2022, Code 5.9].
Also see [GitHub](https://github.com/dynamicslab/databook_python).
:::

The idea of LDA is to find a linear combination of features that optimally separates two or more classes.
The crucial part in the algorithm is that it is guided by labelled observations.
...
::: {.callout-tip}
For supervised learning, it is always good practice to split up our dataset into a _training_ and _testing_ section.
**It is important to have a test set that the algorithm has never seen!**
In general a $80:20$ split is common but other ratios might be advisable, depending on the dataset. 
...
The $k$-folds cross-validation technique is a method to allow better _hyperparameter tuning_, especially for smaller datasets where training and validation data is small.
...
Therefore, we train the model with four parts and validate against the fifth.
We then rotate the folds and select a different one for validation.
At the end, we average over the five iterations to get our final parameters.
...
$$ {#eq-grayleigh}
(the generalized Rayleigh quotient) where $w$ is our thought after projection.
The two included scatter matrices are
$$
...
The two matrices measure the variance of the dataset as well as the means.
...
If we use our raw dataset for the classification we get an overall accuracy of `{python} float(np.round(E, 2))`% with $\tfrac{4}{20}$ wrongly labelled dogs and $\tfrac{9}{20}$ wrongly labelled cats.
...
#| fig-cap: "Cross validation for the dataset in wavelet basis, use 100 run with different training and test sets. We always use 120 images for training and 40 for testing."
#| code-fold: true
#| code-summary: "Show the code for the figure"
import numpy as np
import scipy
...
This allows us to write it also as an update procedure with Bayes[^bayes] theorem.
:::
...
1. Additionally, for a higher dimensional problem, using LDA split the Fischer Iris dataset (see the introduction to part of the notes) into two clusters. Try for the harder split between `versicolor` and `virginica` types of flowers.
::::
:::

[^GEV]: see [@Kandolf_GDM, Definition 3.5] or the direct [Link](https://kandolfp.github.io/MECH-M-DUAL-1-DBM/matrixdc/eigen.html#generalized-eigenvalue-problem)

[^bayes]: see [@Kandolf_GDM, Theorem 14.4] or the direct [Link](https://kandolfp.github.io/MECH-M-DUAL-1-DBM/statistics/bayesian.html#def-statistics-bayesianth)
...
In @fig-clustering-sl-lda2, we implied we are doing better than a coin toss but we should be able to characterize this more precise.
...
In order to illustrate basic properties found in machine learning we use the MNIST dataset together with a binary classifier based on Stochastic Gradient Descent, see @sec-clustering-ul-me for more on the MNIST dataset. 
...
First we load the MNIST dataset again and split it into a training and testing section, see @sec-clustering-ul-me.
...
With the new labels we can train our classifier, compare @def-multiclass-ovr.
...
In order to get a score for our method, we use $k$-folds cross-validation @def-crossvalidation and the corresponding `scikit-learn` function [`cross_val_score`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.cross_val_score.html) to perform this task for our model.
...
With scores in the high $90\%$ range the results look promising, if not great, but are they really that good?
...
As this is pretty much $91\%$ (as expected there are only about $10\%$ of `5`s in the dataset).
...
![Names and abbreviations for a $2\times 2$ confusion matrix together with an example form our test case.](../_assets/clustering/confusion_matrix){#fig-clustering-sl-confusionmatrix width=2.88in}
...
This can be achieved by [`cross_val_predict`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.cross_val_predict.html) instead of `cross_val_score` and than we use [`confusion_matrix`](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.confusion_matrix.html) from `sklearn.metrics`. 
...
[^SGD]: see [@Kandolf_GDM, Section 6.2] or the direct [Link](https://kandolfp.github.io/MECH-M-DUAL-1-DBM/regression/nonlinear.html#stochastic-gradient-descent)
...
They have a long history in data science and come in different forms and fashions.
...
![Hyperplane with small margin.](../_assets/clustering/SVM_hyperplain1){#fig-clustering-sl-svm-1 width=4.70in}

![Hyperplane with large margin.](../_assets/clustering/SVM_hyperplain2){#fig-clustering-sl-svm-2 width=4.68in}
...
On the other hand, this is needed to allow for most optimization algorithm to work, as they require a gradient to some sort. 
...
In order to extend the SVM to more complex classification curves, the feature space can be extended.
...
To illustrate this we use a simple one dimensional example as shown in @fig-clustering-sl-svm-nl-1.
...
As can be seen in @fig-clustering-sl-svm-nl-2 the SVM does a great job in finding a split for the two classes, even though not selecting the _optimal_ line, which is not surprising for the given amount of observations. 
...
::: {.callout-note}
## Pipeline
The main idea of a pipeline is to create a composite as a ordered chain of transformations and estimators, see [docs](https://scikit-learn.org/stable/modules/compose.html) for some more insights.
:::
...
Decision trees are a common tool in data science, for classification and regression.
They are a powerful class of algorithms, that can fit not only numerical data.
Furthermore, they form the basis of _random forests_, one of the most powerful machine learning algorithms available to date.

They where not invented for machine learning but have been a staple in business for centuries.
Their basic idea is to establish an algorithmic flow chart for making decisions.
...
Similar to DBSCAN, they form a hierarchical structure that tries to split _in an optimal way_.
In this regard, they are the counterpart to DBSCAN, as they move from top to bottom and of course use the labels to guide the process. 
...
//| fig-cap: "Decision tree for the Fischer iris dataset with depth 2."
//|file: fischer_tree.dot
```
:::

::: {.content-visible unless-format="html"}
:::: {#fig-clustering-sl-tree-iris fig-env="figure"}
![](../_assets/clustering/fischer_tree.png){width=4.32in}

Decision tree for the Fischer iris dataset with depth 2.
::::
:::
...
In the code for @fig-clustering-sl-tree-iris we generate a `dot` file, that is interpreted with quarto.
This allows a better visual integration.
In order to do the same offline you need to install [`graphviz`](https://www.graphviz.org/download/) for the installation of `dot` and also install the python package `pydotplus`.
...
#| fig-cap: "Splits for the Fischer Iris dataset with the first two split form the above tree and the third split would be the next step for a larger tree."
#| code-fold: true
#| code-summary: "Show the code for the figure"
import matplotlib.pyplot as plt
%config InlineBackend.figure_formats = ["svg"]
...
//| fig-cap: "Decision tree for the dogs and cats dataset with depth 2."
//| file: cvsd_tree.dot
```
:::
::: {.content-visible unless-format="html"}
:::: {#fig-clustering-sl-tree-cvsd fig-env="figure"}

![](../_assets/clustering/cvsd_tree.png){width=5.80in}

Decision tree for the dogs and cats dataset with depth 2.
::::
:::
...
We used the same components before.
...
In @fig-clustering-sl-tree-rot-1 we see the split for the original dataset - random numbers in $[-0.5, 0.5]^2$ and the classes separation for $x_1 > 0$.
A simple line, i.e. one split is enough to make the separation.
If we rotate the dataset by by 45° we can see a way more complicated separation line in @fig-clustering-sl-tree-rot-2.
Furthermore, @fig-clustering-sl-tree-rot-3 shows that the `random_state` has an influence as well.
Finally, if we apply scaling and PCA to the data we more or less end up at our original sample again @fig-clustering-sl-tree-rot-4, showcasing the power of these feature extraction techniques once more.

We note that all the trees make a perfect classification, but the structure is not as easy to recognize as it _could_ be
:::
...
This combinations of trees are called an _ensemble_ and leads to the next topic.
...
Now we select the class which occurs most often, i.e. the _mode_ of the predictions, we get a new classifier.
...
All we need to do, average the probability of the predictions $p_1, \ldots, p_n$, and this will give us new probabilities for our ensemble classifier.
...
![Illustration of the random sampling for bagging and pasting in ensemble classifiers.](../_assets/clustering/ensemble_sampling){#fig-clustering-el-sampling width=3.36in}
...
![Illustration of an $m$ layer stacking with various classifiers in each layer.](../_assets/clustering/ensemble_stacking){#fig-clustering-el-stacking width=4.58in}
...
1. Use the `VotingClassifier` class with an _LDA_, a _SVM_, and a _tree_ for classification. Make sure to set `random_state` for each, to have reproducible results. Get the overall score for the test set, as well as the individual scores for the included classifiers. 
   ```{.python}
...
1. Train a random forest for the Fischer Iris dataset and check `.feature_importances_` to get an insight on the importance of each feature.

1. Train a random forest for our dogs and cats dataset in raw and wavelet form and check `.feature_importances_` to get an insight on the importance of each feature of the PCA.
::::
:::
...
Some of the methods discussed, like random forests, support multiclass classification out of the box.
...
The advantage of OvO over OvR is that each classifier only needs to be trained on a subset and not with the entire dataset. 
...
1. Use SVM for the Fischer Iris dataset and test it.
1. Compare with a random forest.
1. Create a confusion matrix for more than two classes with your results and interpret the results.
::::
:::
